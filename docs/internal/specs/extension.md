...
- contributed command は `workspaceFolderCount == 1` の enablement により VS Code UI 上では disabled になる
- Extension Host は同じ command ID を登録する
- TreeView、watcher、store、editor panel は初期化しない
- 直接 command 実行時は `Launch Composer requires exactly one workspace folder.` を表示する
...
### config file 一括 include/exclude

config file の一括 include/exclude は file 自体の状態を作らない。対象 file の `configurations` 配下にある config entry だけを更新する。

- Include All Configs: 各 config entry から `excluded` key を削除する
- Exclude All Configs: 各 config entry に `excluded: true` を設定する

すでに目的状態の entry は変更しない。変更対象がない場合はファイルを書き換えず、ユーザー向け message も表示しない。
...
config entry の checkbox または `Include` / `Exclude` は entry-level `excluded` を切り替える。config file は生成対象状態を持たず、単なるグループとして扱う。
