...
リクエストとレスポンスの対応付けには `requestId` を使用する。`update-template` / `update-config` を含む、レスポンスを必要とするすべての Webview → Extension Host メッセージは `requestId: string` を持つ。`open-file-json` のみ fire-and-forget とする。
