...
- 状態管理: 編集中は Webview 側の React state がデータのマスターとなる。フォームのうち `name` 以外の変更は debounce（TextInput）または即時（Checkbox・Select・ListEditor）で Extension Host に送信し、Extension Host がファイルに書き込む。`name` は専用の rename 要求で送信する。
- 競合制御: `initial-data` に現在の `editorRevision` を含める。Webview は差分パッチ保存ごとに `baseRevision` を添えて更新を要求し、Extension Host は現在の file revision と一致した場合のみ保存する。
- 競合解決: revision 不一致時は Extension Host が `update-result` で conflict を返し、Webview は最新の `initial-data` を再取得して外部変更を反映する。rename 要求は成功・失敗にかかわらず、その応答後に Webview が最新の `initial-data` を再取得して表示を同期する。
...
4. 名前変更
   ...
   Extension Host: `renameEntry()` を呼び出し、必要なら template 参照先 config の `extends` を更新する
...
リクエストとレスポンスの対応付けには `requestId` を使用する。`update-template` / `update-config` / `rename-entry` を含む、レスポンスを必要とするすべての Webview → Extension Host メッセージは `requestId: string` を持つ。`open-file-json` のみ fire-and-forget とする。
...
Webview 側に、requestId の生成と Promise の管理を行うヘルパー関数を用意する。この関数は内部で requestId を自動付与して `postMessage` を送信し、対応する requestId を持つレスポンスが返ってきたら Promise を resolve する。`update-template` / `update-config` / `rename-entry` もこの経路を使い、結果を明示的に受け取る。
