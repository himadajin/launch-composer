...
- **エディタパネル（編集フォーム）**: React 19 + `@himadajin/vscode-components` で実装した Webview Panel。選択したアイテムのフィールドをフォームで編集する。GUI で扱わないフィールドは `Open JSON` ボタンで対応する JSON ファイルを直接開いて編集する。
...
- **config ファイルアイテム（enabled）**: 通常のファイルアイテムとして表示する。
- **config ファイルアイテム（disabled）**: `$(circle-slash)` アイコンを表示し、description 欄に `disabled` と表示する。
...
- **config エントリ（disabled by file）**: `$(circle-large-outline)` アイコンをグレーで表示し、description 欄に `disabled by file` と表示する。これは config 自体の `enabled` が `false` ではなく、親 file の `enabled` が `false` であることを示す。

インラインアクション（ホバー時のみ表示）:

- **ファイルアイテム**: `$(add)` ボタン — エントリを追加する。
- **config ファイルアイテム**: `Toggle Enabled` ボタン — クリックすると即座にファイルに書き込み、file-level enabled の状態を切り替える。
- **config エントリ**: `Toggle Enabled` ボタン — クリックすると即座にファイルに書き込み、entry-level enabled の状態を切り替える。`disabled by file` のエントリには表示しない。

インラインアクションは VSCode の標準動作に従いホバー時のみ表示する。config の有効/無効状態は file・entry ともに左側のアイコンで常時示す。
...
| テンプレートエントリ追加           | ファイルアイテムの `$(add)` インラインアクション（ホバー時）          | InputBox でテンプレート名を入力（空文字は拒否） → JSON に `name`, `type: ""`, `request: ""` を含む最小構成のエントリを書き込む → エディタパネルを開く（name は確定済み・read-only）                                                                                                       |
| テンプレートエントリを開く         | テンプレートアイテムを右クリック → "Open"                             | 対応する JSON ファイルを VSCode エディタで開き、対象エントリ位置を表示する                                                                                                                                                                                                                |
...
| 構成エントリ追加                   | ファイルアイテムの `$(add)` インラインアクション（ホバー時）          | QuickPick でテンプレートを選択（`(none)` を含む） → InputBox で name 入力（空文字は拒否） → `configurations` 配列に最小構成のエントリを書き込む。`extends` ありなら `enabled: true` と `extends` を含み、`extends` なしならさらに `type: ""`, `request: ""` を含む → エディタパネルを開く |
...
| 構成エントリを開く                 | 構成アイテムを右クリック → "Open"                                     | 対応する JSON ファイルを VSCode エディタで開き、対象エントリ位置を表示する                                                                                                                                                                                                                |
...
| enabled トグル                     | config ファイルまたは構成アイテムのインラインアクション（ホバー時）   | 即座にファイルに書き込み、対象の enabled を切り替える。`disabled by file` の構成アイテムには表示しない                                                                                                                                                                                    |
...
GUI で編集できるフィールドは以下に限定する。それ以外のプロパティ（`env` 等）を変更する場合は、JSON ファイルを直接編集する。

| フィールド        | JSON キー                  | UI コンポーネント       | 備考                                                                     |
| ----------------- | -------------------------- | ----------------------- | ------------------------------------------------------------------------ |
| Name              | `name`（拡張機能固有キー） | read-only `<TextInput>` | フォームでは read-only。名前変更はサイドバーの右クリック `Rename` で行う |
| Type              | `type`                     | `<TextInput>`           | 空欄もそのまま文字列として保持する                                       |
| Request           | `request`                  | `<TextInput>`           | 空欄もそのまま文字列として保持する                                       |
...
`Open JSON` ボタン: 対応する JSON ファイルを VSCode エディタで開き、該当エントリの先頭行にカーソルを移動する（`vscode.window.showTextDocument` + `revealRange`）。

自動保存の動作: フォームの変更は即座に JSON に書き込む（パーシャル更新）。GUI で編集した Type / Request / Program / cwd / stopAtEntry / args のみを差分として送信し、JSON ファイルに存在するその他のキー（`env` 等）はそのまま保持する。
...
- Webview は最新の `editorRevision` を保持し、各保存要求に `baseRevision` を添えて送信する。
- 保存要求は Webview 内で直列化し、前回保存の応答を待ってから次の保存を送る。
- conflict が返った場合は最新の `initial-data` を再取得し、直接編集されたファイル内容で UI を更新する。
...
- `Open JSON` はエントリジャンプではなく、対象ファイル自体を開く。
...
| Request           | `request`                  | `<TextInput>`                 | `extends` なしのときだけ編集可能。`extends` ありのときは参照先テンプレートの値を表示し、config 側には書き込まない                                                                                                                   |
| Working Directory | `cwd`                      | `<TextInput>`                 | 空欄時はキーを JSON から削除                                                                                                                                                                                                        |
| Stop At Entry     | `stopAtEntry`              | `<Checkbox>`                  | 常に `true` / `false` を明示的に書き込む                                                                                                                                                                                            |
...
`extends` を設定した場合、フォームは `type` と `request` をテンプレート継承値として表示する。config に残っていた `type` / `request` は削除し、以後はテンプレート側のみを編集対象とする。`(none)` に戻した場合は config 側の `type` / `request` を空文字 `""` で保持し、standalone config として編集できる状態に戻す。

自動保存の動作: フォームの変更は即座に JSON に書き込む（パーシャル更新）。GUI で編集したフィールドのみを差分として送信し、JSON ファイルに存在するその他のキー（`program`, `env` 等）および file-level `enabled` はそのまま保持する。

- `<TextInput>` フィールド（type, request, cwd, argsFile）は `launch-composer.autoSaveDelay`（デフォルト 1000ms）の debounce を挟んでから書き込む。
...
`Open JSON` ボタンの挙動はテンプレート編集と同様（対応する JSON ファイルを開き、`configurations[index]` の先頭位置にジャンプ）。
...
対象ファイルが invalid な場合の表示原則はテンプレート編集と同じとする。通常の config 編集レイアウトを維持し、`JSON Status` を表示し、すべての編集操作を無効化する。`Open JSON` は対象ファイル自体を開く。
...
| フィールド                                               | コンポーネント                                 |
| -------------------------------------------------------- | ---------------------------------------------- |
| テキスト入力（type, request, program, cwd, argsFile 等） | `<TextInput>`                                  |
