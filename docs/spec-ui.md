...
| view ID                   | 表示名     | 役割                 |
| ------------------------- | ---------- | -------------------- |
| `launchComposer.configs`  | `CONFIGS`  | config file / entry  |
| `launchComposer.profiles` | `PROFILES` | profile file / entry |
...
| -------- | ----------------------------------------------- |
| PROFILES | `No profile files found. [Create Profile File]` |
| CONFIGS  | `No config files found. [Create Config File]`   |
...
| 状態                         | context                     | 表示                                                               |
| ---------------------------- | --------------------------- | ------------------------------------------------------------------ |
| entry enabled、file enabled  | `configEntryEnabled`        | checked checkbox                                                   |
| entry disabled、file enabled | `configEntryDisabled`       | unchecked checkbox、description `disabled`                         |
| file disabled 配下の entry   | `configEntryDisabledByFile` | checkbox なし、`circle-slash` icon、description `disabled by file` |
...
| view     | action                                      |
| -------- | ------------------------------------------- |
| PROFILES | `launch-composer.addProfileFile` (`$(add)`) |
| CONFIGS  | `launch-composer.addConfigFile` (`$(add)`)  |
| CONFIGS  | `launch-composer.generate` (`$(play)`)      |
...
| 表示ラベル                   | JSON path                   | control      | 保存方法                       |
| ---------------------------- | --------------------------- | ------------ | ------------------------------ |
| `Profile: Name`              | `name`                      | `TextInput`  | blur / Enter で rename request |
| `Profile: Program`           | `configuration.program`     | `TextInput`  | debounce 後 patch              |
| `Profile: Working Directory` | `configuration.cwd`         | `TextInput`  | debounce 後 patch              |
| `Profile: Stop At Entry`     | `configuration.stopAtEntry` | `Checkbox`   | 即時 patch                     |
| `Profile: Args`              | `args`                      | `ListEditor` | 変更操作完了時に即時 patch     |
...
| 表示ラベル                  | JSON path                   | control              | 保存方法                                   |
| --------------------------- | --------------------------- | -------------------- | ------------------------------------------ |
| `Config: Name`              | `name`                      | `TextInput`          | blur / Enter で rename request             |
| `Config: Profile`           | `profile`                   | `Select`             | 即時 patch                                 |
| `Config: Enabled`           | `enabled`                   | `Checkbox`           | 即時 patch                                 |
| `Config: Working Directory` | `configuration.cwd`         | `TextInput`          | debounce 後 patch                          |
| `Config: Stop At Entry`     | `configuration.stopAtEntry` | `Checkbox`           | 即時 patch                                 |
| `Config: Args File`         | `argsFile`                  | `TextInput` + Browse | TextInput は debounce、Browse は即時 patch |
| `Config: Args`              | `args`                      | `ListEditor`         | 変更操作完了時に即時 patch                 |
...
| 状態                                           | 表示値/状態                                               |
| ---------------------------------------------- | --------------------------------------------------------- |
| `profile` が空文字、profile 候補あり           | `Select a profile...` placeholder + warning helper        |
| `profile` が string ではない、profile 候補あり | `Select a profile...` placeholder + invalid value warning |
| `profile` が未知文字列、profile 候補あり       | `<name> (missing)` を候補末尾に表示 + warning helper      |
| profile 候補 0 件、`profile` 空または非 string | `No profiles available`、disabled + warning helper        |
| profile 候補 0 件、`profile` が未知文字列      | `<name> (missing)`、disabled + warning helper             |
