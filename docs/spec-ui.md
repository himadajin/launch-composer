...
- **サイドバー（一覧パネル）**: VSCode TreeView API で実装。テンプレートと config の一覧表示・追加・名前変更・削除・有効無効の切り替え・JSON ファイルを開く/パスをコピーするといった基本操作を行う。
- **エディタパネル（編集フォーム）**: React 19 + `@himadajin/vscode-components` で実装した Webview Panel。選択したアイテムのフィールドをフォームで編集する。`env` など GUI で扱わないフィールドは "Edit as JSON" ボタンで対応する JSON ファイルを直接開いて編集する。
...
- **invalid なファイルアイテム**: warning アイコンを表示し、description に `empty file` / `invalid JSON` / `must be array` のいずれかを表示する。子エントリは表示しない。
...
右クリックメニューは VSCode Explorer に近い最小構成にする。項目の追加・削除・名前変更・Open・Copy Path・Copy Relative Path を提供し、config エントリでは `Enable` / `Disable` を最上段に置く。削除はインラインアクションでは提供しない。
...
| 構成エントリ追加                   | ファイルアイテムの `$(add)` インラインアクション（ホバー時）               | QuickPick でテンプレートを選択（`(none)` を含む） → InputBox で name 入力（空文字は拒否） → JSON に最小構成のエントリを書き込む。`extends` ありなら `enabled: true` と `extends` を含み、`extends` なしならさらに `type: ""`, `request: ""` を含む → エディタパネルを開く |
...
| ----------------- | -------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------ |
| Name              | `name`（拡張機能固有キー） | read-only テキスト + `$(settings-gear)` アイコン | フォームでは read-only。名前変更はサイドバーの右クリック `Rename` で行う |
...
- "Edit as JSON" はエントリジャンプではなく、対象ファイル自体を開く。
...
自動保存の動作: フォームの変更は即座に JSON に書き込む（パーシャル更新）。GUI で編集したフィールドのみを差分として送信し、JSON ファイルに存在するその他のキー（`program`, `env` 等）はそのまま保持する。
...
対象ファイルが invalid な場合の表示原則はテンプレート編集と同じとする。通常の config 編集レイアウトを維持し、`JSON Status` を表示し、すべての編集操作を無効化する。"Edit as JSON" は対象ファイル自体を開く。
