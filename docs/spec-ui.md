...
自動保存の動作: フォームの変更は即座に JSON に書き込む（パーシャル更新）。GUI で編集した Type / Request / Program / cwd / stopAtEntry / args のみを書き込み、JSON ファイルに存在するその他のキー（`env` 等）はそのまま保持する。
...
`vscode.workspace.createFileSystemWatcher` で `.vscode/launch-composer/templates/` および `.vscode/launch-composer/configs/` 以下のファイルを監視し、ファイルの作成・変更・削除を検知したらサイドバーのツリーと編集パネルの内容を自動更新する。
