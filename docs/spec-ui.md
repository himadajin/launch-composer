...
- **テンプレートエントリ・config エントリ（全種別）**: `$(gear)` ボタン — エディタパネル（WebUI）で編集画面を開く。`disabled by file` のエントリにも表示する。
...
| テンプレートエントリ編集           | テンプレートアイテムの `$(gear)` インラインアクション（ホバー時）               | エディタパネルで編集画面を開く                                                                                                                                                                                                                                                                  |
...
| 構成エントリ追加                   | ファイルアイテムの `$(add)` インラインアクション（ホバー時）                    | QuickPick でテンプレートを選択（`(none)` を含む） → InputBox で name 入力（空文字は拒否） → `configurations` 配列に最小構成のエントリを書き込む。`extends` ありなら `enabled: true` と `extends` を含み、`extends` なしならさらに `type: ""`, `request: "launch"` を含む → エディタパネルを開く |
...
| 構成エントリ編集                   | 構成アイテムの `$(gear)` インラインアクション（ホバー時）                       | エディタパネルで編集画面を開く                                                                                                                                                                                                                                                                  |
...
`extends` を設定した場合、フォームは `type` と `request` をテンプレート継承値として表示する。config に残っていた `type` / `request` は削除し、以後はテンプレート側のみを編集対象とする。`(none)` に戻した場合は config 側の `type` を空文字 `""`、`request` を `"launch"` に設定し、standalone config として編集できる状態に戻す。
