...
`@launch-composer/webview` および `launch-composer`（extension）の Webview 管理層が実装する UI 仕様を定める。
...
ユーザーは VSCode のサイドバーでprofileと config の一覧を確認し、各アイテムをクリックしてエディタ領域のフォームで詳細を編集する。フォームへの変更は自動的に JSON ファイルに書き込まれる。編集が終わったら CONFIGS ビューヘッダーの Generate ボタンで `.vscode/launch.json` を生成する。

UI は 2 つの独立したコンポーネントで構成する。

- **サイドバー（一覧パネル）**: VSCode TreeView API で実装する。profileと config の一覧表示・追加・名前変更・削除・有効無効の切り替え・JSON ファイルを開く/パスをコピーするといった基本操作を行う。config では file 単位と entry 単位の両方の有効状態を扱う。
- **エディタパネル（編集フォーム）**: React 19 と `@himadajin/vscode-components` で実装した Webview Panel を使用する。選択したアイテムのフィールドをフォームで編集する。GUI で扱わないフィールドは、フォーム最下部の `Edit in <sourceFile>` 導線から対応する JSON ファイルを直接開いて編集する。
...
| エディタ編集画面              | React 19 + @himadajin/vscode-components + Vite 7 | Webview Panel 上のフォーム UI。テーマ変更に自動対応する |
...
Launch Composer の UI は以下の 2 つのコンポーネントで構成する。

| コンポーネント | 配置                          | 技術                                    | 役割                                       |
| -------------- | ----------------------------- | --------------------------------------- | ------------------------------------------ |
| 一覧パネル     | サイドバー                    | VSCode TreeView API                     | profile・config 一覧、操作エントリポイント |
| 編集パネル     | エディタ領域（Webview Panel） | React 19 + @himadajin/vscode-components | 選択アイテムの詳細編集フォーム             |

### 1.3 サイドバー（一覧パネル）: TreeView API

サイドバーパネルに **PROFILES** と **CONFIGS** の 2 つの独立した TreeView を配置する。各 TreeView はそれぞれ折りたたみ・展開できる（VSCode の標準ビュー機能）。
...
- **PROFILES ビュー**: `[+]`（`$(add)` アイコン）— profile ファイルを新規作成
...
- **profileエントリ**: アイコンを表示せず、ラベルのみ表示する。
...
- **profileエントリ・config エントリ（全種別）**: `$(go-to-file)` ボタン — 対応する JSON を 1 クリックで開く。
...
- PROFILES ビューにファイルが存在しない場合、`welcomeContent` でガイダンスを表示する: `No profile files found. [Create Profile File]`
...
| 操作                           | トリガー                                                              | 動作                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------ | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| profileファイル新規作成        | PROFILES ビューヘッダーの `$(add)`                                    | InputBox でファイル名入力 → 空配列 `[]` の JSON ファイルを作成                                                                                                                                                                                                                                                                                                            |
| profileファイルを開く          | ファイルアイテムを右クリック → "Open"                                 | 対応する JSON ファイルを VSCode エディタで開く                                                                                                                                                                                                                                                                                                                            |
| profileファイルのパスをコピー  | ファイルアイテムを右クリック → "Copy Path" / "Copy Relative Path"     | 絶対パスまたはワークスペース相対パスをクリップボードにコピー                                                                                                                                                                                                                                                                                                              |
| profileファイル名変更          | ファイルアイテムを右クリック → "Rename"                               | InputBox でファイル名を変更                                                                                                                                                                                                                                                                                                                                               |
| profileファイル削除            | ファイルアイテムを右クリック → "Delete"                               | 確認ダイアログ → ファイル削除                                                                                                                                                                                                                                                                                                                                             |
| profileエントリ追加            | ファイルアイテムの `$(add)` インラインアクション（ホバー時）          | InputBox でprofile名を入力（空文字は拒否） → JSON に `name` と `configuration: { type: "", request: "launch" }` を含む最小構成のエントリを書き込む → エディタパネルを開く                                                                                                                                                                                                 |
| profileエントリを開く          | profileアイテムをクリック、またはprofileアイテムを右クリック → "Open" | 対応する JSON ファイルを VSCode エディタで開き、対象エントリの配列要素位置にカーソルを移動・選択状態にする                                                                                                                                                                                                                                                                |
| profileエントリのパスをコピー  | profileアイテムを右クリック → "Copy Path" / "Copy Relative Path"      | 対応する JSON ファイルの絶対パスまたはワークスペース相対パスをクリップボードにコピー                                                                                                                                                                                                                                                                                      |
| profileエントリ名変更          | profileアイテムを右クリック → "Rename"                                | InputBox で `name` を変更する                                                                                                                                                                                                                                                                                                                                             |
| profileエントリ削除            | profileアイテムを右クリック → "Delete"                                | 確認ダイアログ → 配列から要素を削除してファイルに書き戻す。そのprofileを参照する config エントリが存在する場合は削除を拒否しエラーを通知する                                                                                                                                                                                                                              |
| profileエントリ編集            | profileアイテムをクリック                                             | エディタパネルで編集画面を開く                                                                                                                                                                                                                                                                                                                                            |
| profileエントリの JSON を開く  | profileアイテムの `$(go-to-file)` インラインアクション（ホバー時）    | 対応する JSON ファイルを VSCode エディタで開き、対象エントリの配列要素位置にカーソルを移動・選択状態にする                                                                                                                                                                                                                                                                |
...
| 構成エントリ追加               | ファイルアイテムの `$(add)` インラインアクション（ホバー時）          | 利用可能な profile を先に読み込む。profile が 1 件以上ある場合は QuickPick で profile を選択 → InputBox で name 入力（空文字は拒否） → `configurations` 配列に `enabled: true` と `profile` を含む最小構成のエントリを書き込む → エディタパネルを開く。profile が 0 件の場合は config を作らず、`Create a profile before adding a config.` と `Create Profile` を表示する |
...
ファイル名は、VSCode がサポートする各 OS（Linux・Windows・macOS）で有効な名前であれば受理する。拡張機能側で独自の追加制限は設けない。
...
- VS Code の editor tab には現在編集中の項目名だけを表示する。`Profile` / `Config` の種別名や `Edit` のような操作名は表示しない。
- tab title は、panel を開いた時・rename が成功した時・workspace 同期で対象名が変わった時に再計算する。
...
GUI で編集できるフィールドは以下に限定する。その他のプロパティ（`env` を含む）を変更する場合は、フォーム最下部の `Edit in <sourceFile>` 導線から JSON ファイルを直接編集する。

| フィールド        | JSON キー                  | UI コンポーネント | 備考                                                                                                    |
| ----------------- | -------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------- |
| Name              | `name`（拡張機能固有キー） | `<TextInput>`     | blur または Enter 確定時に rename 要求を送る。profile を参照する config の `profile` は同時に更新される |
...
保存の動作: フォームの変更はエントリ単位で JSON に反映する。`name` は専用の rename 要求で保存し、それ以外の GUI 編集項目はパーシャル更新として保存する。JSON ファイルに存在するその他のキー（`env` を含む）は保持する。

- `Name` は blur または Enter 確定時に rename 要求を送る。自動保存の debounce は使用しない。
- `Program` / `Working Directory` の `<TextInput>` は `launch-composer.autoSaveDelay`（デフォルト 1000ms）の debounce を挟んでから差分を書き込む。
...
- 通常のprofile編集レイアウトを維持し、フォーム先頭に `JSON Status` の行を表示する。
...
GUI で編集できるフィールドは以下に限定する。その他のプロパティ（`program` や `env` を含む）を変更する場合は、フォーム最下部の `Edit in <sourceFile>` 導線から JSON ファイルを直接編集する。

| フィールド        | JSON キー                  | UI コンポーネント             | 備考                                                                                                                                                                                                                              |
| ----------------- | -------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Name              | `name`（拡張機能固有キー） | `<TextInput>`                 | blur または Enter 確定時に rename 要求を送る                                                                                                                                                                                      |
| Profile           | `profile`                  | `<Select>`                    | 利用可能な profile 名を列挙する。空白 option は表示しない。値が空または不正なら `Select a profile...`、未知の文字列なら `<name> (missing)` を表示する。profile が 0 件のときは `No profiles available` を表示して disabled にする |
| Enabled           | `enabled`                  | `<Checkbox>`                  | 常に `true` / `false` を明示的に書き込む。新規追加時の初期値は `true`                                                                                                                                                             |
| Working Directory | `cwd`                      | `<TextInput>`                 | 空欄時はキーを JSON から削除                                                                                                                                                                                                      |
| Stop At Entry     | `stopAtEntry`              | `<Checkbox>`                  | 常に `true` / `false` を明示的に書き込む                                                                                                                                                                                          |
| Args File         | `argsFile`                 | `<TextInput>` + Browse ボタン | 空欄時はキーを JSON から削除。選択中profileに `args` が定義されている場合は disabled にし、`The selected profile already defines args.` と表示する                                                                                |
| Args              | `args`                     | `<ListEditor>`                | 空リスト時はキーを JSON から削除                                                                                                                                                                                                  |

config 編集フォームでは `type` / `request` / `program` を表示しない。これらは profile 側または JSON 直接編集で扱う。

保存の動作: フォームの変更はエントリ単位で JSON に反映する。`name` は専用の rename 要求で保存し、それ以外の GUI 編集項目はパーシャル更新として保存する。JSON ファイルに存在するその他のキー（`program` や `env` を含む）および file-level `enabled` は保持する。

- `Name` は blur または Enter 確定時に rename 要求を送る。自動保存の debounce は使用しない。
- `<TextInput>` フィールド（Working Directory, Args File）は `launch-composer.autoSaveDelay`（デフォルト 1000ms）の debounce を挟んでから差分を書き込む。
- `<Checkbox>`（enabled, stopAtEntry）・`<Select>`（profile）は変更と同時に即座に書き込む（debounce なし）。
- `<ListEditor>`（args）は要素の追加・削除・並び替えの各操作完了時に即座に書き込む（debounce なし）。
- **debounce はユーザーが TextInput を編集した場合にのみ発火する。エディタを開いた時点（初期表示）では発火しない。** これは VS Code の設定エディタが採用する原則と同様である: 「値をセットしてからハンドラを登録する」＝プログラム的な値の更新はファイル書き込みをトリガーしない。
- `configuration` 内のすべての省略可能フィールドが削除された結果 `configuration` が空になった場合、`configuration` キー自体を JSON から削除する。
- Webview は差分更新用に最新の `editorRevision` を保持し、各差分保存要求に `baseRevision` を添えて送信する。
...
フォーム最下部の `Edit in <sourceFile>` 導線の挙動はprofile編集と同様とする。対応する JSON ファイル全体を開き、エントリ位置へのジャンプは行わない。
...
- profile が 1 件以上あり、`config.profile` が未指定・空文字・非 string の場合は `Select a profile...` を表示し、warning helper で修正を促す。
- `config.profile` が非空文字列だが既存 profile に一致しない場合は `<name> (missing)` を表示し、壊れた参照を明示する。
- profile が 0 件の場合は selector を disabled にし、`No profiles available` を表示する。
- 上記のどの場合も、空白ラベルの option は表示しない。

対象ファイルが invalid な場合の表示原則はprofile編集と同じとする。通常の config 編集レイアウトを維持し、`JSON Status` を表示し、すべての編集操作を無効化する。`JSON Status` 内の `Edit in <sourceFile>` リンクは対象ファイル自体を開く。
...
| フィールド                             | コンポーネント                                 |
| -------------------------------------- | ---------------------------------------------- |
| テキスト入力（program, cwd, argsFile） | `<TextInput>`                                  |
| 列挙選択（profile）                    | `<Select>`                                     |
...
エディタパネルは**単一パネル方式**とする。サイドバーのアイテムをクリックするたびに同じ Webview Panel の内容が切り替わる。パネルは常に最大 1 枚とし、複数の編集画面を同時に開く手段は提供しない。
...
`vscode.workspace.createFileSystemWatcher` で `.vscode/launch-composer/profiles/` および `.vscode/launch-composer/configs/` 以下のファイル変更を反映し、サイドバーのツリーと編集パネルの内容を自動更新する。

issue 通知は watcher 経由の再読込時に評価するが、同一 file の同一 issue は 1 回だけ通知し、正常化したら通知状態を破棄する。
